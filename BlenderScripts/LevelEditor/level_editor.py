...
class MYADDON_OT_export_scene(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    def perse_scene_recursive_json(self, data_parent, object, level):
        ...
        if len(object.children) > 0:
            ...
            # 子ノードへ進む(深さが1上がる)
            for child in object.children:
                self.perse_scene_recursive_json(json_object["children"], child, level + 1)
    ...
    def export_json(self):
        ...
        for object in bpy.context.scene.objects:
            ...
            self.perse_scene_recursive_json(json_object_root["objects"], object, 0)

        json_text = json.dumps(
            json_object_root,
            ensure_ascii=False, # 日本語をエスケープせずにそのまま出力
            cls=json.JSONEncoder, # デフォルトのエンコーダを指定
            indent=4, # インデントをスペース4つ分にする
        )
        # コンソールに表示してみる
        print(json_text)
