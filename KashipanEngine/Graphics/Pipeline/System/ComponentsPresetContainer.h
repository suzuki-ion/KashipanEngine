...
public:
    ...
    //--------- RootSignature ---------//
    void RegisterRootSignature(const std::string &name, const D3D12_ROOT_SIGNATURE_DESC &desc) { rootSignatures_.push_back(name, desc); }
    ...
    [[nodiscard]] const D3D12_ROOT_SIGNATURE_DESC &GetRootSignature(const std::string &name) const { return rootSignatures_.at(name).value; }
