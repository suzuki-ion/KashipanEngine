...
#include "Graphics/RenderPassesComponent.h"
...
void Renderer::RenderWorld(WorldECS &world) {
    const auto &entities = world.GetEntitiesWithComponents<RenderPassesComponent>();
    if (entities.empty()) return;

    std::vector<const RenderPassInfo *> passes;
    for (const auto &entity : entities) {
        auto *component = world.GetComponent<RenderPassesComponent>(entity);
        if (!component) continue;
        for (const auto &pass : component->passes) {
    ...
    std::stable_sort(passes.begin(), passes.end(), [](const RenderPassInfo *a, const RenderPassInfo *b) {
        const bool aHasMesh = a && a->mesh.IsValid();
        const bool bHasMesh = b && b->mesh.IsValid();
        if (aHasMesh != bHasMesh) return !aHasMesh;
        if (a->priority != b->priority) return a->priority < b->priority;
        if (a->pipelineName != b->pipelineName) return a->pipelineName < b->pipelineName;
    ...
    struct EcsBatchKey {
        ...
        bool operator==(const EcsBatchKey &o) const {
            return targetType == o.targetType && target == o.target && mesh == o.mesh
                && texture == o.texture && pipelineName == o.pipelineName;
        }
    };

    struct EcsBatchKeyHasher {
        size_t operator()(const EcsBatchKey &k) const noexcept {
            size_t h1 = std::hash<int>{}(static_cast<int>(k.targetType));
            size_t h2 = std::hash<void *>{}(k.target);
            size_t h3 = std::hash<std::string>{}(k.pipelineName);
            size_t h4 = std::hash<std::size_t>{}(k.mesh.id);
            size_t h5 = std::hash<void *>{}(k.texture);
            size_t h = h1;
    ...
    std::unordered_map<EcsBatchKey, std::vector<const RenderPassInfo *>, EcsBatchKeyHasher> batches;

    for (const auto *pass : passes) {
        if (!pass || !pass->mesh.IsValid()) {
            continue;
        }

        if (!pass->target.IsValid() || pass->pipelineName.empty()) {
            continue;
        }

        EcsBatchKey key{ pass->target.type, pass->target.target, pass->pipelineName, pass->mesh, pass->texture };
        batches[key].push_back(pass);
    }

    for (const auto &batch : batches) {
        const auto &entries = batch.second;
        if (entries.empty()) continue;
        const size_t instanceCount = entries.size();
        (void)instanceCount;
    }
}
