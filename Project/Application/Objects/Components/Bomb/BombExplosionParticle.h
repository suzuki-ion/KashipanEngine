...
namespace KashipanEngine {
    ...
    class BombExplosionParticle final : public IObjectComponent3D {
    public:

        BombExplosionParticle(const ParticleConfig& config = {});

        ~BombExplosionParticle() override = default;

        std::unique_ptr<IObjectComponent> Clone() const override {
            return std::make_unique<BombExplosionParticle>(config_);
        }
