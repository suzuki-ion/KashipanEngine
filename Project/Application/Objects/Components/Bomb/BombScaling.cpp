    ...
    std::optional<bool> BombScaling::Update() {
        ...
        if (elapsedBeats >= maxBeats - 1) {
            ...
            if (countBeats_ >= 3) {
                ...
                targetScale = Eased(maxSpeedScale_, detonationScale_, speed3ScaleTimer_.GetProgress(), easeType_);
            } else {
                targetScale = Eased(minSpeedScale_, maxSpeedScale_, speed2ScaleTimer_.GetProgress(), easeType_);
            }
        ...
        // 最終拍の1つ前の場合は素早くスケーリング
            ...
            targetScale = Eased(minSpeedScale_, maxSpeedScale_, speedScaleTimer_.GetProgress(), easeType_);
        }
        // 通常の拍
        else {
            targetScale = Eased(minScale_, maxScale_, bpmProgress_, easeType_);
        }
