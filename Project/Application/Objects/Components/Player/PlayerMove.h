    ...
    private:
        ...
        void UpdateKnockback() {
            if (enableDestructiveKnockback_) {
                ...
                // イージングを使って滑らかに吹き飛ぶ
                Vector3 currentPos = Vector3(Eased(knockbackStartPosition_, knockbackTargetPosition_, t, EaseType::EaseOutExpo));

                // 高さの補間
                float currentPosY = float(EasedGAB(0.0f, 0.5f, t, EaseType::EaseOutCirc, EaseType::EaseInCirc));
        ...
        void IsMove() {
            ...
            if (isOutOfBounds_) {
                ...
                currentPos = Vector3(EasedGAB(startPosition_, outOfBoundsTarget, t));
            } else {
                // 通常の移動
                currentPos = Vector3(Eased(startPosition_, targetPosition_, t, EaseType::EaseOutExpo));
            }
