...
PuzzleBlockFaller::PuzzleBlockFaller(PuzzleBoard *board, PuzzleBlockPlacePositions *placePositions, PuzzleBlockNextContainer *nextContainer)
    : ISceneComponent("PuzzleBlockFaller", 2), board_(board), placePositions_(placePositions), nextContainer_(nextContainer) {}

PuzzleBlockFaller::~PuzzleBlockFaller() = default;

void PuzzleBlockFaller::Initialize() {
    currentColumn_ = 0;
...
bool PuzzleBlockFaller::MoveRight() {
    if (!board_) return false;
    if (currentBlockData_.empty() || currentBlockData_.front().empty()) return false;
...
bool PuzzleBlockFaller::RotateLeft() {
...
bool PuzzleBlockFaller::RotateRight() {
    if (currentRotationStep_ >= 1) return false;
    ++currentRotationStep_;
    RebuildCurrentBlockByRotation();
    ClampColumnByShape();
...
bool PuzzleBlockFaller::PlaceCurrentBlock() {
...
const std::vector<std::vector<PuzzleBlockData>> &PuzzleBlockFaller::GetCurrentBlockData() const { return currentBlockData_; }

void PuzzleBlockFaller::SpawnNext() {
    DestroyTriangles(placedPreviewTriangles_);
    DestroyTriangles(ghostTriangles_);
...
void PuzzleBlockFaller::RebuildCurrentBlockByRotation() {
...
void PuzzleBlockFaller::ClampColumnByShape() {
...
void PuzzleBlockFaller::RefreshPlacePreview() {
...
void PuzzleBlockFaller::CreateVisualTriangles(const std::vector<std::vector<PuzzleBlockData>> &shape,
...
void PuzzleBlockFaller::DestroyTriangles(std::vector<KashipanEngine::Triangle2D *> &targets) {
...
size_t PuzzleBlockFaller::CountCurrentBlockCells() const {
    size_t count = 0;
