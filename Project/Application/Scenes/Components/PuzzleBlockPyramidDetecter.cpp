...
PuzzleBlockPyramidDetecter::PuzzleBlockPyramidDetecter(PuzzleBoard *board)
    : ISceneComponent("PuzzleBlockPyramidDetecter", 2), board_(board) {}

PuzzleBlockPyramidDetecter::~PuzzleBlockPyramidDetecter() = default;

void PuzzleBlockPyramidDetecter::Initialize() {
    detectedPyramids_.clear();
    detectedIndexGroups_.clear();
    ClearHighlightObjects();
}

void PuzzleBlockPyramidDetecter::Finalize() {
    ClearHighlightObjects();
    detectedPyramids_.clear();
    detectedIndexGroups_.clear();
}

void PuzzleBlockPyramidDetecter::Update() {
...
const std::vector<DetectPyramidData> &PuzzleBlockPyramidDetecter::GetDetectedPyramids() const { return detectedPyramids_; }

const std::vector<std::vector<std::pair<size_t, size_t>>> &PuzzleBlockPyramidDetecter::GetDetectedIndexGroups() const { return detectedIndexGroups_; }

void PuzzleBlockPyramidDetecter::DetectPyramids() {
    detectedPyramids_.clear();
    detectedIndexGroups_.clear();
...
bool PuzzleBlockPyramidDetecter::TryBuildPyramidUp(int apexX, int apexY, int level, PuzzleBlockType type, WorkPyramidData &out) const {
...
bool PuzzleBlockPyramidDetecter::TryBuildPyramidDown(int apexX, int apexY, int level, PuzzleBlockType type, WorkPyramidData &out) const {
    out.data.outerIndices.clear();
    out.data.innerIndices.clear();
    out.allIndices.clear();
...
bool PuzzleBlockPyramidDetecter::IsSubset(
...
void PuzzleBlockPyramidDetecter::RefreshHighlights() {
    ClearHighlightObjects();
...
void PuzzleBlockPyramidDetecter::ClearHighlightObjects() {
    auto *ctx = GetOwnerContext();
    if (!ctx) {
