        ...
        if (currentDisplayedWave_ >= 1 && currentDisplayedWave_ <= kMaxWaveNumbers_) {
            ...
            if (waveNumbers_[index]) {
                if (auto* tr = waveNumbers_[index]->GetComponent3D<Transform3D>()) {
                    // イージングを適用した補間
                    Vector3 position = Eased(waveDisplayStartPosition_, waveDisplayEndPosition_, t, EaseType::EaseInBack);
                    Vector3 rotation = Eased(waveDisplayStartRotate_, waveDisplayEndRotate_ + Vector3({ 6.28f,0.0f,0.0f }), t, EaseType::EaseInExpo);
        ...
        if (currentDisplayedWave_ >= 1 && currentDisplayedWave_ <= kMaxWaveNumbers_) {
            ...
            if (waveNumbers_[index]) {
                if (auto* tr = waveNumbers_[index]->GetComponent3D<Transform3D>()) {
                    ...
                    Vector3 position = Eased(waveDisplayEndPosition_, waveDisplayStartPosition_, t, EaseType::EaseOutExpo);
                    Vector3 rotation = Eased(waveDisplayEndRotate_, waveDisplayStartRotate_, t, EaseType::EaseOutExpo);
                    tr->SetTranslate(position);
                    tr->SetRotate(rotation);
                }
            }
