                ...
                if (walls_[z][x].object) {
                    ...
                    if (auto* tr = walls_[z][x].object->GetComponent3D<Transform3D>()) {
                        if (walls_[z][x].moveTimer.IsActive()) {
                            tr->SetTranslateY(Eased(-2.1f, 0.0f, walls_[z][x].moveTimer.GetProgress()));
                        }
    ...
    // OnUpdate 内で BPM 進行度を更新
    {
        if (bpmObjects_[0] && bpmObjects_[1]) {
            ...
            if (bpmSystem_->GetLeftRightToggle()) {
                tr->SetTranslate(Vector3(Eased(bpmObjectStart_[0], bpmObjectEnd_[0], bpmSystem_->GetBeatProgress())));
                tr1->SetTranslate(bpmObjectStart_[1]);
            } else {
                tr1->SetTranslate(Vector3(Eased(bpmObjectStart_[1], bpmObjectEnd_[1], bpmSystem_->GetBeatProgress())));
