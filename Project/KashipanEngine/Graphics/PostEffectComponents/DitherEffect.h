...
class DitherEffect final : public IPostEffectComponent {
public:
    struct Params {
        float intensity = 1.0f; // 輝度に対するスケーリング
        bool  color = true;     // 色を保持するか（falseなら単色）
    };

    explicit DitherEffect(Params p = {})
        : IPostEffectComponent("DitherEffect", 1), params_(p) {}

    void SetParams(const Params &p) { params_ = p; }
    const Params &GetParams() const { return params_; }

    std::unique_ptr<IPostEffectComponent> Clone() const override {
        return std::make_unique<DitherEffect>(params_);
    }

#if defined(USE_IMGUI)
    void ShowImGui() override {
        ImGui::DragFloat("Intensity", &params_.intensity, 0.01f, 0.0f, 8.0f);
    ...
    std::vector<PostEffectPass> BuildPostEffectPasses() const override {
        ...
        pass.passName = "Dither";
        pass.batchKey = 0;

        pass.constantBufferRequirements = {
            {"Pixel:DitherCB", sizeof(CBData)}
        };
