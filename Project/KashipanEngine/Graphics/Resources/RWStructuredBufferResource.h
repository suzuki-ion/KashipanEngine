...
public:
    ...
    RWStructuredBufferResource(size_t elementStride, size_t elementCount);

    /// @brief リソース再生成（要素数・ストライドが変わった場合に呼ぶ）
    /// @return 成功した場合はtrue、失敗した場合はfalseを返す
    bool Recreate(size_t elementStride, size_t elementCount);
...
private:
    bool Initialize(size_t elementStride, size_t elementCount);
