...
public:
    ...
    std::unique_ptr<IObjectComponent> Clone() const override {
        ...
        ptr->size_ = size_;
        ptr->center_ = center_;
        ptr->autoGenerated_ = autoGenerated_;
    ...
    /// @brief TilemapRenderer等が自動生成したコライダーかどうかを設定する
    /// @details 生成元コンポーネント側が、シーンリロードをまたいでも自分が過去に生成した
    ///          コライダーだけを安全に再識別・削除できるようにするためのフラグ。
    ///          （追跡用にComponentRef等をランタイムのみで保持すると、フルリロードで
    ///          追跡側のインスタンスごと消えてしまい、JSONには残った生成済みコライダーを
    ///          孤立させてしまう。このフラグ自体をJSONへ永続化することでその問題を避ける）
    void SetAutoGenerated(bool autoGenerated) noexcept { autoGenerated_ = autoGenerated; }
    bool IsAutoGenerated() const noexcept { return autoGenerated_; }
...
protected:
    ...
    JSON SaveToJson() const override {
        ...
        json["autoGenerated"] = autoGenerated_;
        return json;
    }
    bool LoadFromJson(const JSON &json) override {
        ...
        autoGenerated_ = json.value("autoGenerated", false);
        return true;
    }
