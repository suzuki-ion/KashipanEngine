...
class SSAOEffect final : public IPostProcessComponent {
public:
    struct Params {
    ...
    SSAOEffect() : IPostProcessComponent("SSAOEffect", GetComponentTypeID<SSAOEffect>()) {
...
protected:
    ...
    bool RenderCustom(CustomRenderContext &context) override {
        ...
        static const char *kSSAO = "PostEffect.SSAO";
        static const char *kBlur = "PostEffect.SSAO.Blur";
        static const char *kComposite = "PostEffect.SSAO.Composite";
        if (!context.pipelineManager->HasPipeline(kSSAO) ||
        ...
        // AO計算パス: 深度のみからワールド座標・法線を再構成して遮蔽率を求める
        {
            ...
            cbData.texelSize[0] = (ssaoRaw_.width > 0) ? (1.0f / static_cast<float>(ssaoRaw_.width)) : 0.0f;
            cbData.texelSize[1] = (ssaoRaw_.height > 0) ? (1.0f / static_cast<float>(ssaoRaw_.height)) : 0.0f;
            cbData.rotationSeed = 0.0f;
            if (!ssaoConstantBuffer_) ssaoConstantBuffer_ = std::make_unique<ConstantBufferResource>(sizeof(SSAOParamsCBData));
            if (void *mapped = ssaoConstantBuffer_->Map()) {
                std::memcpy(mapped, &cbData, sizeof(cbData));
            }

            drawTo(ssaoRaw_, kSSAO, [&](ShaderVariableBinder &binder) {
                binder.Bind("Pixel:SSAOParamsCB", ssaoConstantBuffer_.get());
        ...
        // ブラーパス: 深度差を考慮してAOのノイズを除去する
        {
            ...
            if (!blurConstantBuffer_) blurConstantBuffer_ = std::make_unique<ConstantBufferResource>(sizeof(SSAOBlurCBData));
            if (void *mapped = blurConstantBuffer_->Map()) {
                std::memcpy(mapped, &cbData, sizeof(cbData));
            }

            drawTo(ssaoBlurred_, kBlur, [&](ShaderVariableBinder &binder) {
                binder.Bind("Pixel:SSAOBlurCB", blurConstantBuffer_.get());
                binder.Bind("Pixel:gTexture", ssaoRaw_.srv->GetGPUDescriptorHandle());
                binder.Bind("Pixel:gDepthTexture", screenBuffer->GetDepthSrvHandle());
        ...
        binder.Bind("Pixel:gSSAOTexture", ssaoBlurred_.srv->GetGPUDescriptorHandle());
        SamplerManager::BindSampler(&binder, "Pixel:gSampler", DefaultSampler::LinearClamp);
        commandList->DrawInstanced(3, 1, 0, 0);
...
private:
    /// @brief SSAOParamsCB（SSAOCB.hlsli）と同レイアウトの構造体
    struct SSAOParamsCBData {
    ...
    /// @brief SSAOBlurCB（SSAOBlurCB.hlsli）と同レイアウトの構造体
    struct SSAOBlurCBData {
        float texelSize[2]{};
        int radius = 2;
    ...
    bool EnsureIntermediateTargets(ScreenBuffer *owner) {
        ...
        return ssaoRaw_.rt && ssaoRaw_.srv && ssaoBlurred_.rt && ssaoBlurred_.srv;
    }

    Params params_{};
    std::unique_ptr<ConstantBufferResource> ssaoConstantBuffer_;
    std::unique_ptr<ConstantBufferResource> blurConstantBuffer_;
    EffectRT ssaoRaw_;
    EffectRT ssaoBlurred_;
    /// @brief 直近のRenderCustomでカメラが解決できたか（ImGuiでの状態表示用）
    bool lastCameraValid_ = true;
};

REGISTER_COMPONENT_OBJECT(SSAOEffect)

// AmbientOcclusionEffect名で保存された既存シーンをSSAOEffectへ移行するための読込専用エイリアス。
static const bool isAmbientOcclusionEffectAliasRegistered = RegisterObjectComponentTypeAlias(
    "AmbientOcclusionEffect",
    []() -> std::unique_ptr<IObjectComponent> { return std::make_unique<SSAOEffect>(); });
