...
Ring2D::Ring2D(size_t segmentCount, float innerRadius, float outerRadius, float startAngle, float endAngle, UvMode uvMode)
    : Object2DBase("Ring2D", sizeof(Vertex), sizeof(Index), (segmentCount + 1) * 2, segmentCount * 6),
...
void Ring2D::SetInnerRadius(float innerRadius) {
    innerRadius_ = innerRadius;
    UpdateVertices();
}

void Ring2D::SetOuterRadius(float outerRadius) {
    outerRadius_ = outerRadius;
    UpdateVertices();
}

void Ring2D::SetStartAngle(float startAngle) {
    startAngle_ = startAngle;
    UpdateVertices();
}

void Ring2D::SetEndAngle(float endAngle) {
    endAngle_ = endAngle;
    UpdateVertices();
}

void Ring2D::SetUvMode(UvMode uvMode) {
    uvMode_ = uvMode;
    UpdateVertices();
}

void Ring2D::UpdateVertices() {
...
bool Ring2D::Render([[maybe_unused]] ShaderVariableBinder &shaderBinder) {
...
std::optional<RenderCommand> Ring2D::CreateRenderCommand(PipelineBinder &pipelineBinder) {
