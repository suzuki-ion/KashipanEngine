...
class ModelAnimator final : public ISceneComponent {
public:
    struct TimelineKey {
        float time = 0.0f;
        float value = 0.0f;
    ...
    /// @brief `ModelAnimator` を破棄する
    ~ModelAnimator() override = default;
    ...
    bool AddPresetObject(const std::string &presetName, const std::string &objectName, const std::string &parentObjectName = "");

    /// @brief プリセットから対象モデル情報を削除する
    /// @param presetName バインディングプリセット名
    /// @param objectName 対象オブジェクト名
