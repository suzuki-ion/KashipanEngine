...
/// @return 補間されたベクトル/値
template<typename T>
T Eased(const T& start, const T& end, float t, EaseType type = EaseType::Linear) {
...
template<typename T>
T EasedGAB(const T &start, const T &end, float t,
    ...
    if (t < 0.5f) {
        ...
        return Eased<T>(start, end, normalizedT, goType);
    } else {
        // 帰り：end → start
        float normalizedT = (t - 0.5f) * 2.0f;  // 0.5〜1 → 0〜1
        return Eased<T>(end, start, normalizedT, backType);
    }
}
