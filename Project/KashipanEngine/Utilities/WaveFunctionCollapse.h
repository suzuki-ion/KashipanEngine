...
public:
    ...
    struct Tile final {
        /// @brief タイルを識別するID
        std::uint32_t id = 0;
        /// @brief 各方向ごとに接続可能なタイルIDの一覧
        std::array<std::vector<std::uint32_t>, kDirectionCount> connections;
    };
    ...
    /// @return 登録に成功した場合はtrue（同じIDのタイルが既に登録済みの場合はfalse）
    bool RegisterTile(const Tile &tile);

    /// @brief 接続情報を持たない空のタイルを登録する（登録後にAddTileConnectionで接続を追加する用途）
    /// @return 登録に成功した場合はtrue（同じIDのタイルが既に登録済みの場合はfalse）
    bool RegisterTile(std::uint32_t id);

    /// @brief 登録済みタイルの、指定方向の接続先タイルIDを1つ追加する
    /// @param tileID 接続を追加するタイルのID
    /// @param direction 接続方向
    /// @param connectedTileID その方向に接続可能とするタイルのID
    /// @return 追加できた場合はtrue（タイルが未登録の場合はfalse。既に追加済みの場合は何もせずtrue）
    bool AddTileConnection(std::uint32_t tileID, Direction direction, std::uint32_t connectedTileID);

    /// @brief 登録済みのタイルを削除する
    /// @details 削除対象のタイルで固定されていたグリッド上のセルは固定解除される
    /// @param tileID 削除するタイルのID
    /// @return 削除できた場合はtrue（該当タイルが存在しない場合はfalse）
    bool RemoveTile(std::uint32_t tileID);

    /// @brief 登録済みのタイル一覧を取得する
    const std::unordered_map<std::uint32_t, Tile> &GetTiles() const noexcept { return tiles_; }

    /// @brief 指定した座標のタイルを固定する
    /// @param x,y,z 固定するグリッド座標
    /// @param tileID 固定するタイルのID
    /// @return 固定できた場合はtrue（座標が範囲外、またはタイルが未登録の場合はfalse）
    bool FixTile(std::uint32_t x, std::uint32_t y, std::uint32_t z, std::uint32_t tileID);

    /// @brief 指定した座標に固定されているタイルIDを取得する
    /// @return 固定されていない、または座標が範囲外の場合はstd::nullopt
    std::optional<std::uint32_t> GetFixedTile(std::uint32_t x, std::uint32_t y, std::uint32_t z) const;
    ...
    /// @brief Solve()によって確定したタイルIDを取得する
    /// @return 未確定（Solve未実行・矛盾で失敗・座標が範囲外）の場合はstd::nullopt
    std::optional<std::uint32_t> GetResolvedTile(std::uint32_t x, std::uint32_t y, std::uint32_t z) const;
...
private:
    /// @brief グリッド上の1マス分の状態
    struct Cell final {
        /// @brief 固定されているタイルID（未固定の場合はstd::nullopt）
        std::optional<std::uint32_t> fixedTileID;
        /// @brief Solve()によって確定したタイルID（未確定の場合はstd::nullopt）
        std::optional<std::uint32_t> resolvedTileID;
    };
    ...
    /// @brief 公開API経由で登録されたタイル群
    std::unordered_map<std::uint32_t, Tile> tiles_;
